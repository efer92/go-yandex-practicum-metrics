...
- При обращении к PostgreSQL сервер получил ошибку транспорта (из категории Class 08 — Connection Exception).
...
## Инкремент 15
Перепланируйте архитектуру агента таким образом, чтобы сбор метрик (опрос runtime) и их отправка осуществлялись в разных горутинах. 
При этом количество одновременно исходящих запросов на сервер нужно ограничивать «сверху».
...
В директории cmd создайте пакет linter.
В данную директорию добавьте исходный код статического анализатора, запускаемого с помощью golang.org/x/tools/go/analysis/singlechecker.
...
Логика работы анализатора должена быть покрыта тестами с использованием пакета golang.org/x/tools/go/analysis/analysistest.
```
...
Необходимо реализовать структуру с generic-параметром, которая сможет хранить в себе объекты одного конкретного типа.
