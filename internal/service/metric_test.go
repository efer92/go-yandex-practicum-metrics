...
func TestMetricService_UpdateMetric(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            err := svc.UpdateMetric(tt.mType, tt.mName, tt.value)
...
func TestMetricService_UpdateMetric_Logic(t *testing.T) {
    ...
    svc.UpdateMetric(model.Counter, "count", "10")
    svc.UpdateMetric(model.Counter, "count", "5")
    val, _ := storage.GetValue(model.Counter, "count")
    if val != "15" {
        t.Errorf("expected counter 15, got %s", val)
    }

    // Gauge должен замещаться
    svc.UpdateMetric(model.Gauge, "gauge", "10.5")
    svc.UpdateMetric(model.Gauge, "gauge", "20.5")
    val, _ = storage.GetValue(model.Gauge, "gauge")
    if val != "20.5" {
...
func TestMetricService_GetValue(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            got, err := svc.GetValue(tt.mType, tt.mName)
            if (err != nil) != tt.wantErr {
